...
genki4k_image_list = [f for f in os.listdir(CONSTANT.GENKI4K_db_path) if
                      os.path.isfile(os.path.join(CONSTANT.GENKI4K_db_path, f))]
...
for image_name in genki4k_image_list:
    try:
        ...
        if result is not None:
            ...
            face_index = np.argmax(total_boxes[:, 4], axis=0)
            ...
            ''' Pick the bigger (or with higher probability?) face '''
            cv2.imwrite('./aligned_face_4/' + image_name, chips[face_index])
            # for i, chip in enumerate(chips):
            #     # cv2.imshow('chip_' + str(i), chip)
            #     # cv2.imwrite('./aligned_face/chip_' + str(i) + '.png', chip)
            #     cv2.imwrite('./aligned_face_2/' + image_name, chip)
